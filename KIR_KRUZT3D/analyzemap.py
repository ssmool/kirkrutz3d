...
# ---------- Blender lithophane emitter (writes the final script file) ----------
BLENDER_LITHO_SCRIPT = r'''# image_to_3d_blender_lithophane.py
...
def emit_blender_script(path="image_to_3d_blender_lithophane.py"):
    with open(path, "w", encoding="utf-8") as f:
        f.write(BLENDER_LITHO_SCRIPT)
    return path
...
def main():
    ...
    if args.emit_blender:
        path = emit_blender_script("image_to_3d_blender_lithophane.py")
        print("Emitted Blender script:", path)
        print("Run in Blender: blender --background --python image_to_3d_blender_lithophane.py --", out_json, "--image-dir . --out-dir ./blender_out --sample 4 --height-scale 0.02 --rgb-tolerance 12 --solidify 0.6 --export-format obj")
    print("Done.")
